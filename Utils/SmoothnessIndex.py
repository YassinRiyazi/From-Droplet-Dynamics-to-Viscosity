"""
    ...
    https://github.com/Ahmadkalhor/data_complexity_measures
"""
import torch
import numpy as np
...
# 8-To learn a model in a forward layer-wise manner (using a ranking loss (to maximize the SmI) at each layer, one can get better results than conventional back propagation methods)
...
class Kalhor_SmoothnessIndex:
    """
    Kalhor Smoothness Index (SmI) Class.

    This class provides a collection of 29 methods for computing smoothness
    indices (SmI) and their variants on regression datasets. The Smoothness Index
    is a normalized supervised data complexity measure designed by Dr. Ahmad Kalhor
    to analyze and evaluate AI models, datasets, and features in regression contexts.

    The methods compute different forms of SmI (linear, exponential, high-order,
    soft-order, anti-smoothness, cross-smoothness, triplet-based, and input-ranking).
    Each variant captures different aspects of smoothness and generalization ability
    of data, models, or features.

    ---
    Core Concepts:
        - **Smoothness Index (SmI)**: A normalized score in [0,1] reflecting how
          "smooth" the relationship is between inputs and targets. Higher values
          indicate simpler, more predictable relationships.
        - **Variants**: Linear vs. exponential, higher-order approximations, soft-order
          averaging, anti-SMI (detecting complexity), cross-SMI (generalization across
          datasets), and ranking-based feature selection.

    ---
    Applications:
        1. Dataset evaluation (low SmI → harder dataset; high SmI → easier dataset).
        2. Layer-wise analysis of deep models (expected SmI increase across layers).
        3. Model ranking (higher SmI indicates better performance).
        4. Generalization assessment (cross-SMI).
        5. Feature selection and ranking.
        6. Data cleaning (detect trojans/backdoors/biases).
        7. Model compression (remove layers/nodes with no SmI gain).
        8. Forward layer-wise training with SmI-based ranking loss.
        9. Identifying key branching/fusion layers in models.
        10. Trustworthy/confident predictions (via high/soft-order SmIs).
        11. Data augmentation and removal of risky points.
    ...
    Args:
        ...
        target (torch.Tensor):
            Target tensor of shape (n_samples, n_outputs).
        inp_normalize (bool, optional):
            Whether to normalize input features before processing. Default: False.
        target_normalize (bool, optional):
            Whether to normalize target values before processing. Default: False.

    Attributes:
        inp (torch.Tensor): Possibly normalized input data.
        target (torch.Tensor): Possibly normalized target data.
        dis_matrix_input (torch.Tensor): Pairwise input distances.
        dis_matrix_target (torch.Tensor): Pairwise target distances.
        dis_target_max (torch.Tensor): Maximum pairwise target distance per sample.
        dis_target_min (torch.Tensor): Minimum pairwise target distance per sample.
        dis_target_mean (torch.Tensor): Mean pairwise target distance per sample.
        pdist (torch.nn.PairwiseDistance): Pairwise L2 distance function.
        n_data (int): Number of samples.
        dim_input (int): Number of input features.
        dim_target (int): Number of target dimensions.

    ---
    Example:
        >>> import torch
        >>> from kalhor_smi import Kalhor_SmoothnessIndex
        >>> X = torch.randn(100, 5)
        >>> y = torch.randn(100, 1)
        >>> smi = Kalhor_SmoothnessIndex(X, y, inp_normalize=True, target_normalize=True)
        >>> score = smi.smi_linear()
        >>> print(score)
        tensor(0.63)

    Reference:
        Kalhor, Ahmad. *Smoothness Index for Regression Problems*. (2023).
    """
    ...
    # (1. smi_linear method)==========================================
    def smi_linear(self) -> torch.Tensor:
        """
        Compute the **linear Smoothness Index (SmI)**.

        This metric measures smoothness based on the nearest neighbor in input space,
        normalized using the minimum and maximum distances in target space.

        ---
        Returns:
            torch.Tensor:
                Scalar tensor containing the average linear smoothness index in [0,1].

        Example:
            >>> smi = Kalhor_SmoothnessIndex(X, y)
            >>> score = smi.smi_linear()
            >>> print(score)
            tensor(0.72)
        """
    ...
    def smi_linear_data(self):
        """
        Compute the **per-sample linear SmI values**.

        Unlike `smi_linear`, which returns the dataset-wide average, this method
        returns a vector of SmI scores for each data point.

        ---
        Returns:
            torch.Tensor:
                Tensor of shape (n_samples,) containing individual smoothness scores.

        Example:
            >>> smi = Kalhor_SmoothnessIndex(X, y)
            >>> values = smi.smi_linear_data()
            >>> values.shape
            torch.Size([100])
        """
        values, indices = torch.min(self.dis_matrix_input, 0)
    ...
    def smi_exp(self, gama):
        """
        Compute the **exponential Smoothness Index (SmI)**.

        Uses an exponential decay weighting with hyperparameter `gama` to capture
        smoothness based on relative distances.

        ---
        Args:
            gama (float):
                Smoothing hyperparameter. Higher values penalize large differences
                more strongly.

        Returns:
            torch.Tensor:
                Scalar tensor containing the average exponential smoothness index in [0,1].

        Example:
            >>> smi = Kalhor_SmoothnessIndex(X, y)
            >>> score = smi.smi_exp(gama=0.5)
            >>> print(score)
            tensor(0.61)
        """
    ...
    def high_order_smi_linear(self, order):
        """
        Compute the **high-order linear Smoothness Index (SmI)**.

        Instead of considering only the nearest neighbor, this method considers the
        `order` closest neighbors in input space and computes the minimum smoothness
        value across them.

        ---
        Args:
            order (int):
                Number of neighbors to consider in the input space.

        Returns:
            torch.Tensor:
                Scalar tensor containing the high-order linear smoothness index.

        Example:
            >>> smi = Kalhor_SmoothnessIndex(X, y)
            >>> score = smi.high_order_smi_linear(order=5)
            >>> print(score)
            tensor(0.78)
        """
    ...
    def cross_smi_linear(self, inp_test, target_test):
        """
        Compute the **cross linear Smoothness Index (SmI)**.

        This evaluates generalization by measuring smoothness between
        a test dataset `(inp_test, target_test)` and the training dataset.

        ---
        Args:
            inp_test (torch.Tensor):
                Test inputs of shape (n_test, n_features).
            target_test (torch.Tensor):
                Test targets of shape (n_test, n_outputs).

        Returns:
            torch.Tensor:
                Scalar tensor containing the cross-dataset linear smoothness index.

        Example:
            >>> smi = Kalhor_SmoothnessIndex(X_train, y_train)
            >>> score = smi.cross_smi_linear(X_test, y_test)
            >>> print(score)
            tensor(0.54)
        """
